...
type Tab = "overview" | "menu" | "ingredients" | "checklist" | "staff" | "qr" | "customers" | "checkins" | "team" | "branches";
...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    async function guardAndLoad() {
      ...
      const { data: restaurant } = await supabase
        .from("restaurants")
        .select("name, status, plan, default_menu_language")
  ...
  async function addStaffCode(role: "waiter" | "kitchen" | "barista") {
    ...
    if (role !== "barista" && currentCount >= limit) return;
  ...
  if (restaurantStatus === "paused" || restaurantStatus === "cancelled") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f6f7f9] px-4">
        <div className="bg-white border border-neutral-200 rounded-2xl p-8 max-w-sm text-center shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          <div className="w-12 h-12 rounded-full bg-red-50 flex items-center justify-center mx-auto mb-4">
            <span className="text-xl">🔒</span>
          </div>
          <h1 className="text-lg font-semibold text-neutral-900 mb-2">
            {restaurantStatus === "cancelled" ? "Account cancelled" : "Account paused"}
          </h1>
          <p className="text-sm text-neutral-500 mb-1">
            {restaurantStatus === "cancelled"
              ? "This account has been cancelled and no longer has access."
              : "This account is temporarily paused, usually due to a pending payment."}
          </p>
          <p className="text-sm text-neutral-500">Please contact your platform administrator to resolve this.</p>
        </div>
      </div>
    );
  }
        ...
        {activeTab === "team" && myRole === "owner" && (
          <section>
            ...
            <form onSubmit={addManager} className={`${card} p-4 mb-3 grid grid-cols-3 gap-2`}>
              ...
              <input placeholder="Password" className={input} value={newManager.password} onChange={(e) => setNewManager({ ...newManager, password: e.target.value })} />
