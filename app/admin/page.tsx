...
export default function AdminDashboard() {
  ...
  async function loadRestaurants() {
    ...
    for (const r of overdue) {
      await supabase.rpc("admin_set_restaurant_billing", { p_restaurant_id: r.id, p_status: "paused" });
    }
    if (overdue.length > 0) {
  ...
  async function markPaid(id: string, cycle: string | null) {
    ...
    await supabase.rpc("admin_set_restaurant_billing", {
      p_restaurant_id: id,
      p_status: "active",
      p_last_paid_at: new Date().toISOString().split("T")[0],
      p_next_billing_date: next.toISOString().split("T")[0],
    });
    loadRestaurants();
  }
