...
export default function RestaurantDetail() {
  ...
  const [chargeCycle, setChargeCycle] = useState("monthly");
  ...
  async function updateStatus(status: string) {
    // plan/status/billing columns are locked to service-role + this RPC now --
    // a direct .update() here would silently no-op against the DB.
    const { error } = await supabase.rpc("admin_set_restaurant_billing", { p_restaurant_id: id, p_status: status });
    setStatusMsg(error ? error.message : `Status set to ${status}`);
    if (!error) setRestaurant((r) => (r ? { ...r, status } : r));
  ...
  async function resetPassword(e: React.FormEvent) {
    ...
    try {
      ...
      setResetMsg(res.ok ? "Password updated ✓" : result.error ?? "Failed to reset password");
  ...
  // Sends the owner a Tap-hosted checkout link for their current plan/cycle,
  // rather than charging a card directly -- there is no saved card on file to
  // charge server-side, and Tap requires a source/token from the payer's own
  // card entry, so a bare amount+currency charge here would always fail.
  async function sendPaymentLink(e: React.FormEvent) {
    e.preventDefault();
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/create-checkout`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ restaurantId: id, plan: restaurant.plan, billingCycle: chargeCycle }),
      });
      const result = await res.json();
      if (!res.ok) { setChargeMsg(result.error ?? "Could not create checkout link"); return; }
      await navigator.clipboard.writeText(result.redirectUrl);
      setChargeMsg(`Link copied ✓ (BHD ${Number(result.amount).toFixed(3)}) — send it to the owner.`);
    } catch {
  ...
  const statusColor: Record<string, string> = {
    ...
    paused: "bg-neutral-200 text-neutral-600", cancelled: "bg-red-100 text-red-700",
  };
    ...
    <div className="min-h-screen bg-[#f6f7f9] px-6 py-8 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className={`${card} p-4`}>
          ...
          <form onSubmit={sendPaymentLink} className="flex gap-2">
            <select value={chargeCycle} onChange={(e) => setChargeCycle(e.target.value)} className="border border-neutral-300 rounded-lg px-2 py-1.5 text-xs flex-1">
            ...
            <button disabled={charging} className="text-xs bg-neutral-900 text-white rounded-lg px-3 py-1.5 disabled:opacity-50">{charging ? "Creating..." : "Copy payment link"}</button>
          </form>
